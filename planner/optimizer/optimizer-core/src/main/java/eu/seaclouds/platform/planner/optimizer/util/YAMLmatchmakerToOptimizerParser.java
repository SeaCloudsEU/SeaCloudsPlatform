...
public class YAMLmatchmakerToOptimizerParser {
   ...
   private static SuitableOptions getSuitableCloudOptionsAndCharacteristicsForModules(String suitableCloudOffers) {
      ...
      for (Object moduleObject : listCompleteOffers) {
         ...
         for (Map.Entry<String, Object> moduleEntry : module.entrySet()) {
            ...
            try {
               // This method relies on the fact that a concrete map is always
               // iterated in the same order (whathever it was)
               if (log.isDebugEnabled()) {
                  log.debug("exploring the options for module" + moduleEntry.getKey());
               }
               List<String> listOfferNames = YAMLmatchmakerToOptimizerParser
                     .getNamesOfSuitableOptions((List<Map<String, Object>>) moduleEntry.getValue());
               options.addSuitableOptions(moduleEntry.getKey(), listOfferNames, YAMLmatchmakerToOptimizerParser
                     .getSuitableOffersOfModule((List<Map<String, Object>>) moduleEntry.getValue(), listOfferNames));
   ...
   private static List<CloudOffer> getSuitableOffersOfModule(List<Map<String, Object>> listOptions,
         List<String> listOfferNames) {
   ...
   private static CloudOffer getAllCharacteristicsOfCloudOffer(String offerName,
      ...
      CloudOffer offer = getAllCharacteristicsOfSingleCloudOffer(offerName, cloudOfferInfoMap);

      return offer;

   }

   private static CloudOffer getAllCharacteristicsOfSingleCloudOffer(String offerName,
         Map<String, Object> cloudOfferInfoMap) {
      CloudOffer offer;
      ...
      try {
         ...
         offer.setAvailability(
               getPropertyOfCloudOffer(TOSCAkeywords.CLOUD_CONCRETE_OFFER_AVAILABILITY, cloudOfferInfoMap));

         offer.setPerformance(
               getPropertyOfCloudOffer(TOSCAkeywords.CLOUD_CONCRETE_OFFER_PERFORMANCE, cloudOfferInfoMap));
   ...
   private static Map<String, Object> getCloudOfferInfoMapFromCloudOfferListByName(
         List<Map<String, Object>> listOptions, String offerName) {
   ...
   public static Map<String, Object> getOfferInformationOfModule(String moduleName, String cloudOfferNameForModule,
      ...
      for (Object moduleObject : listCompleteOffers) {
         ...
         for (Map.Entry<String, Object> moduleEntry : module.entrySet()) {
            ...
            if (moduleEntry.getKey().equals(moduleName)) {
               return getCloudOfferInfoMapFromCloudOfferListByName((List<Map<String, Object>>) moduleEntry.getValue(),
                     cloudOfferNameForModule);
            }
         }
      }
   ...
   /**
    * @param benchmarkPlatformsTOSCA
    *           is in TOSCA-like YAML and offers are in the node_templates
    * @return a map structure with the cloud offers indexed by their name.
    */
   public static Map<String, CloudOffer> createBenchmarkPlatforms(String benchmarkPlatformsTOSCA) {
      Map<String, Object> allStructure = YAMLoptimizerParser.getMAPofAPP(benchmarkPlatformsTOSCA);
      Map<String, Object> nodes = YAMLoptimizerParser.getModuleMapFromAppMap(allStructure);

      Map<String, CloudOffer> offersMap = new HashMap<String, CloudOffer>();

      for (Map.Entry<String, Object> entry : nodes.entrySet()) {

         try {
            CloudOffer offer = YAMLmatchmakerToOptimizerParser.getAllCharacteristicsOfSingleCloudOffer(entry.getKey(),
                  (Map<String, Object>) entry.getValue());
            if (offer.getPerformance() > 0) {
               offersMap.put(entry.getKey(), offer);
            }
         } catch (ClassCastException e) {
