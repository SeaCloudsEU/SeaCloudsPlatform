	...
	public class HillClimb  extends AbstractHeuristic implements SearchMethod  {
		...
		public Map<String, Object>[] computeOptimizationProblem(SuitableOptions cloudOffers,	Map<String, Object> applicationMap, Topology topology, int numPlansToGenerate) {
			...
			//This way may help for replanning, when even the first attempt for solution will be based on the current deployment
			 ...
			 Solution[] bestSols = findInitialSolutions(cloudOffers, applicationMap,numPlansToGenerate);			 
			 ...
			 while(numItersNoImprovement<super.getMaxIterNoImprove()){ //each iteration finds a peak
				 ...
				 while(neighborsImprove){
					 ...
					 else{//there is not any better neighbor. See if this solution can be included among the set of best solutions
						 neighborsImprove=false;
					 }
				 }
				 
				 //We have a local maxima in currentSol, let's see if it is better than the previous maxima. 
		...
		/**
		 * @param currentSol 
		 * @param cloudOffers- Sorted by performance
		 * @param applicationMap
		 * @param topology 
		 * @return an array of solutuions that are neighbors of currentSol
		 */
		private Solution[] findNeighbors(Solution currentSol,SuitableOptions cloudOffers, Map<String, Object> applicationMap, Topology topology) {
			...
			for(String modulename : currentSol){
				...
				//Same cloud provider and same machines, change the number of instances of one module (+1 o -1 (in case there is more than one))			 
				 findNeighborsByNumInstances(neighbors,currentSol,modulename,topology);
				 
				//Same cloud provider, change the type of machine with the same instances (+1 o -1 if sorted by peformance or cost)
				...
				//Change the cloud provider (to +1 -1 in availability) and choose the most similar type of machines for them as the currently 
		...
		private void findNeighborsByCloudProvider(ArrayList<Solution> neighbors, Solution currentSol,String modulename, SuitableOptions cloudOffers) {
			...
			//+1
			 if(cloudOffers.existsAlternativeCloudProviderForModuleWithHigherAvailability(modulename,currentCloudOffer)){
				 ...
				 neighborSol.modifyCloudOfferOfModule(modulename,cloudOffers.getOfferImmediateHigherAvailabilityOfSameProviderSimilarPerformance(modulename,currentCloudOffer));
			 ...
			 if(cloudOffers.existsAlternativeCloudProviderForModuleWithLowerAvailability(modulename,currentCloudOffer)){
				 neighborSol= currentSol.clone();
				 neighborSol.modifyCloudOfferOfModule(modulename,cloudOffers.getOfferImmediateLowerAvailabilityOfSameProviderSimilarPerformance(modulename,currentCloudOffer));
				 neighbors.add(neighborSol);
			 }			
		}

		private void findNeighborsByTypeInstanceInSameProvider(ArrayList<Solution> neighbors, Solution currentSol,String modulename, SuitableOptions cloudOffers) {
			...
			String currentCloudOffer = currentSol.getCloudOfferNameForModule(modulename);
			 ...
			 if(cloudOffers.existsOfferWithBetterPerformanceOfSameProvider(modulename,currentCloudOffer)){
				 ...
				 neighborSol.modifyCloudOfferOfModule(modulename,cloudOffers.getOfferImmediateHigherPerformanceOfSameProvider(modulename,currentCloudOffer));
				 neighbors.add(neighborSol);
			 }
			 //-1
			 if(cloudOffers.existsOfferWithWorsePerformanceOfSameProvider(modulename,currentCloudOffer)){
				 neighborSol= currentSol.clone();
				 neighborSol.modifyCloudOfferOfModule(modulename, cloudOffers.getOfferImmediateLowerPerformanceOfSameProvider(modulename,currentCloudOffer));
				 neighbors.add(neighborSol);
			 }
		 ...
		 * @param currentSol
		 * @param modulename
		 * Uses same cloud provider and same machines, change the number of instances of one module (+1 o -1 (in case there is more than one))
		 * @param topology 
		 */
		private void findNeighborsByNumInstances(ArrayList<Solution> neighbors,	Solution currentSol, String modulename, Topology topology) {
			Solution neighborSol; 
			
			 //+1
			 //TODO: Here it should be checked if there is a maximum number of replicas allowed for a module (see calculation thresholds)
			 //At this point it is a boolean that specifies whether a module can scale out. 
		...
		private Solution[] findInitialSolutions(SuitableOptions cloudOffers, Map<String, Object> applicationMap,int numPlansToGenerate) {
