...
public class YAMLmodulesOptimizerParser {
	...
	public static boolean ModuleHasModuleRequirements(Object module,Map<String, Object> modulesMap) {
		 ...
		 Map<String, Object> moduleReqs=null;
		   ...
		   //If requirements=1, it could be its host so check that there are at least two requirements
		   ...
		   for (Map.Entry<String, Object> entry : moduleReqs.entrySet()){
			   if(!entry.getKey().equals(TOSCAkeywords.MODULE_REQUIREMENTS_HOST)){
				   ...
				   try{ //It may happen that the result is not a String but a Hashmap... in that case just skip the check for this entry
	...
	/**
	 * @param value. The string to check whether it is a name of one of the modules.
	 * @param modulesMap. A MAP of the information of all modules, being the key of the map teh module names
	 * @return
	 */
	...
	public static String getMeasuredPerformanceHost(Map<String, Object> module) {
		 ...
		 Map<String, Object> moduleReqs=null;
		   try{
			  ...
			  if(!moduleInfo.containsKey(TOSCAkeywords.MODULE_QOS_PROPERTIES)){
			   ...
			   moduleReqs = (Map<String, Object>) moduleInfo.get(TOSCAkeywords.MODULE_QOS_PROPERTIES);
			   
			   //Check Existence of measured host
			   if(moduleReqs.containsKey(TOSCAkeywords.MODULE_QOS_PERFORMANCE_LOCATION)){
				  return (String) moduleReqs.get(TOSCAkeywords.MODULE_QOS_PERFORMANCE_LOCATION);
			   }
		 ...
		 //not found There were qos properties but not the information of teh machine tested
		   return null;
	}
