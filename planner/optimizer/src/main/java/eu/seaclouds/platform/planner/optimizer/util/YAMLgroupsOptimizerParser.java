...
public class YAMLgroupsOptimizerParser {
   ...
   public static boolean GroupHasQoSRequirements(Object group) {
      ...
      try {
         ...
         groupReqs = YAMLgroupsOptimizerParser.getPolicySubInfoFromGroupInfo(groupInfo, TOSCAkeywords.GROUP_POLICY_QOSREQUIREMENTS);
     ...
     /* Map<String, Object> groupInfo = null;
      ...
      for (Object listElement : groupPoliciesList) {
         ...
         if (mapEntryListElement.containsKey(TOSCAkeywords.GROUP_POLICY_QOSREQUIREMENTS)) {
            // found QoS requirements in a module
            log.info("  Check successful, it has quality requirements");
            return true;
         }
      }
   ...
   public static Map<String, Object> getDependenciesInfoOfMemberName(String moduleName, Map<String, Object> groups) {
      Map<String, Object> groupInfo = YAMLgroupsOptimizerParser.findGroupOfMemberName(moduleName, groups);
      return YAMLgroupsOptimizerParser.getPolicySubInfoFromGroupInfo(groupInfo, TOSCAkeywords.GROUP_POLICY_DEPENDENCIES);
   }
   ...
   public static List<String> getListDependentGroupsOfModule(String moduleName, Map<String, Object> groups) {
      ...
      if (dependeciesInfoOfGroupOfModule == null) {
         if (IS_DEBUG) {
            log.info("There has not been found info of dependencies for module called " + moduleName + " returning null");
         }
         return null;
      }

      if (dependeciesInfoOfGroupOfModule.containsKey(TOSCAkeywords.GROUP_ELEMENT_DEPENDENCIES_MODULES_TAG)) {
         return (List<String>) dependeciesInfoOfGroupOfModule
   ...
   public static double getReceivedWorkloadOfGroup(Object group) {
      ...
      if(qosInformation.containsKey(TOSCAkeywords.GROUP_POLICY_QOSREQUIREMENTS_WORKLOAD_MINUTE)){
         ...
         try{
         workloadInfo = (Map<String, Object>) qosInformation.get(TOSCAkeywords.GROUP_POLICY_QOSREQUIREMENTS_WORKLOAD_MINUTE);
         }catch (ClassCastException E) {
 ...
 public static Map<String,Object> getQoSInformationInPolicies(Object group){
     ...
     /* for (Object listElement : groupPoliciesList) {
         ...
         Map<String, Object> qosInformation=null;
         try {
            if (!mapEntryListElement.containsKey(TOSCAkeywords.GROUP_POLICY_QOSREQUIREMENTS)) {
               // found QoS requirements in a module
               log.info("  Check of existence of QoS requirements unsuccessful");
               return null;
            }
            return (Map<String, Object>) mapEntryListElement.get(TOSCAkeywords.GROUP_POLICY_QOSREQUIREMENTS);
            
         }
         catch (ClassCastException E) {
            log.info("  Casting of map of '"+TOSCAkeywords.GROUP_POLICY_QOSREQUIREMENTS+"' has not been successful");
         }
      
      }
      log.warn("not QoS information found in the elements of the group. Returning null");
      return null;*/
   }
}
