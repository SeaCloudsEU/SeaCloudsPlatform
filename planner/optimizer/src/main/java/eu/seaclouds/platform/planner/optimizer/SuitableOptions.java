 ...
 *
	...
	*/
	...
	private void sortDescendingPerformanceOptionsForIthModule(int listIndex) {
		...
		List<CloudOffer> listToSort = suitableOptionsCharacteristics.get(listIndex);
...
/////////////////////////////
	class CloudOptionPerformanceComparator implements Comparator<CloudOffer>{
		...
		public int compare(CloudOffer o1, CloudOffer o2) {
			return (int) ((o1.getPerformance()*COMPARATOR_LIMIT) - (o2.getPerformance()*COMPARATOR_LIMIT));
		}
		
	}
	
	class CloudOptionReversePerformanceComparator implements Comparator<CloudOffer>{

		@Override
		public int compare(CloudOffer o1, CloudOffer o2) {
			return (int) ((o2.getPerformance()*COMPARATOR_LIMIT)-(o1.getPerformance()*COMPARATOR_LIMIT) );
		}
...
/////////////////////////////
	 ...
	 * @param modulename
	 * @param cloudOffer
	 * @return whether exists a worse offer in terms of performance of the same provider than cloudOffer. 
	 * 	 */
	public boolean existsOfferWithWorsePerformanceOfSameProvider(String modulename, String cloudOffer) {
	 ...
	 * @return The offer in terms of performance of the same provider that is immediately worse than cloudOffer. 
	 * It is assumed that ArraysOfcloudOffer are ordered by "CloudOptionReversePerformanceComparator"
	 */
	public CloudOffer getOfferImmediateLowerPerformanceOfSameProvider(String modulename, String cloudOffer) {
		List<CloudOffer> offers = getCloudOffersForModule(modulename);
		CloudOffer currentOffer = getCloudCharacteristicsFromList(offers,cloudOffer);
		
		
		
		for(CloudOffer offer : offers){ //assumed that are ordered in reverse order
			if(offer.getPerformance()<currentOffer.getPerformance()){
				if(offer.getProviderName().equals(currentOffer.getProviderName())){
					return offer; //The first one in an ordered traverse must be the chosen one
				}
			}
		}
	 ...
	 * @param modulename
	 * @param currentCloudOffer
	 * @return whether exists a better offer in terms of performance of the same provider than cloudOffer. 
	 * It is assumed that ArraysOfcloudOffer are ordered by "CloudOptionReversePerformanceComparator"
	 */
	public boolean existsOfferWithBetterPerformanceOfSameProvider(String modulename, String cloudOffer) {
	 ...
	 * @param modulename
	 * @param currentCloudOffer
	 * @return The offer in terms of performance of the same provider that is immediately better than cloudOffer. 
	 * It is assumed that ArraysOfcloudOffer are ordered by "CloudOptionReversePerformanceComparator"
	 */
	public CloudOffer getOfferImmediateHigherPerformanceOfSameProvider(	String modulename, String cloudOffer) {
		List<CloudOffer> offers = getCloudOffersForModule(modulename);
		CloudOffer currentOffer = getCloudCharacteristicsFromList(offers,cloudOffer);
	...
	public boolean existsAlternativeCloudProviderForModuleWithHigherAvailability(String modulename, String cloudOffer) {
		List<CloudOffer> offers = getCloudOffersForModule(modulename);
		CloudOffer currentOffer = getCloudCharacteristicsFromList(offers,cloudOffer);
	...
	public CloudOffer getOfferImmediateHigherAvailabilityOfSameProviderSimilarPerformance(String modulename, String cloudOffer) {
		List<CloudOffer> offers = getCloudOffersForModule(modulename);
		CloudOffer currentOffer = getCloudCharacteristicsFromList(offers,cloudOffer);
		...
		for(CloudOffer offer : offers){ //assumed that are ordered in reverse order
			if(offer.getAvailability()>currentOffer.getAvailability()){
				if(!(offer.getProviderName().equals(currentOffer.getProviderName()))){
					...
					else{//An alternative offer was already found, now check by its performance (the less difference)
						if(Math.abs(currentOffer.getPerformance()-offer.getPerformance()) 
											< 
							(Math.abs(currentOffer.getPerformance()-potentialBetter.getPerformance()))){
								//Closest difference
									potentialBetter=offer;
								}
					}
	...
	public boolean existsAlternativeCloudProviderForModuleWithLowerAvailability(String modulename, String cloudOffer) {
		List<CloudOffer> offers = getCloudOffersForModule(modulename);
		CloudOffer currentOffer = getCloudCharacteristicsFromList(offers,cloudOffer);
	...
	public CloudOffer getOfferImmediateLowerAvailabilityOfSameProviderSimilarPerformance(String modulename, String cloudOffer) {
		List<CloudOffer> offers = getCloudOffersForModule(modulename);
		CloudOffer currentOffer = getCloudCharacteristicsFromList(offers,cloudOffer);
	...
	private CloudOffer getCloudCharacteristicsFromList(List<CloudOffer> offers,	String cloudOffer) {
		boolean found=false;
		int indexCloudOffer=0;
