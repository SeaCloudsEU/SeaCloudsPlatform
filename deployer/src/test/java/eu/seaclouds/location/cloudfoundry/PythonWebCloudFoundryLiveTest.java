...
import org.apache.brooklyn.entity.cloudfoundry.webapp.python.PythonCloudFoundryPaasWebApp;
...
public class PythonWebCloudFoundryLiveTest extends AbstractCloudFoundryPaasLocationLiveTest {

    private final String APPLICATION_URL = checkNotNull(getClass().getClassLoader()
            .getResource("pythonHelloWorld")).getFile();


    @Test(groups = {"Live"})
    protected void deployApplicationTest() throws Exception {
        final PythonCloudFoundryPaasWebApp server = app.
                createAndManageChild(EntitySpec.create(PythonCloudFoundryPaasWebApp.class)
        ...
        Asserts.succeedsEventually(new Runnable() {
            public void run() {
                ...
                assertTrue(server.getAttribute(PythonCloudFoundryPaasWebApp
                        .SERVICE_PROCESS_IS_RUNNING));

                assertNotNull(server.getAttribute(Attributes.MAIN_URI));
                assertNotNull(server.getAttribute(PythonCloudFoundryPaasWebApp.ROOT_URL));

                assertEquals(server.getAttribute(PythonCloudFoundryPaasWebApp.DISK),
                        PaasLocationConfig.REQUIRED_DISK.getDefaultValue());
                assertEquals(server.getAttribute(PythonCloudFoundryPaasWebApp.INSTANCES_NUM),
                        PaasLocationConfig.REQUIRED_INSTANCES.getDefaultValue());
                assertEquals(server.getAttribute(PythonCloudFoundryPaasWebApp.MEMORY),
    ...
    protected void stopApplicationTest() throws Exception {
        ...
        Asserts.succeedsEventually(new Runnable() {
            public void run() {
                assertTrue(server.getAttribute(Startable.SERVICE_UP));
                app.stop();
                assertEquals(server.getAttribute(PythonCloudFoundryPaasWebApp
                        .SERVICE_STATE_ACTUAL), Lifecycle.STOPPED);
                assertFalse(server.getAttribute(Startable.SERVICE_UP));
                assertNull(server.getAttribute(PythonCloudFoundryPaasWebApp
                        .SERVICE_PROCESS_IS_RUNNING));
            }
        });
    }
